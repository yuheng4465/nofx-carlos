...
type AutoTrader struct {
	...
	callbackRate          float64                      //追踪止损回调比例，可取值范围[0.1, 10],其中 1代表1%
	activationPriceRate   float64                      //激活追踪止损价格百分比，相对止盈价格
...
func NewAutoTrader(config AutoTraderConfig, database interface{}, userID string) (*AutoTrader, error) {
	...
	if systemPromptTemplate == "" {
		...
		systemPromptTemplate = "adaptive_v2"
	}
...
func (at *AutoTrader) runCycle() error {
	...
	currentPositionsAfterExecution, err := at.trader.GetPositions(false)
	if err != nil {
...
func (at *AutoTrader) executeUpdateStopLossWithRecord(decision *decision.Decision, actionRecord *logger.DecisionAction) error {
	...
	if err := at.trader.CancelStopOrders(decision.Symbol, "stop"); err != nil {
...
func (at *AutoTrader) executeUpdateTakeProfitWithRecord(decision *decision.Decision, actionRecord *logger.DecisionAction) error {
	...
	if err := at.trader.CancelStopOrders(decision.Symbol, "take_profit"); err != nil {
...
func (at *AutoTrader) executePartialCloseWithRecord(decision *decision.Decision, actionRecord *logger.DecisionAction) error {
	...
	if positionSide == "LONG" {
		order, err = at.trader.CloseLong(decision.Symbol, closeQuantity, true)
	} else {
		order, err = at.trader.CloseShort(decision.Symbol, closeQuantity, true)
	}
