...
// 转换为字符串
func getWMADataString(wmaData []*WMAData, period int) string {
	var data []float64
	// 取尾部数据
	startIndex := len(wmaData) - period
	if startIndex < 0 {
		startIndex = 0 // 如果数据不足10条，则从0开始取
	}
	lastData := wmaData[startIndex:]
	for _, v := range lastData {
		data = append(data, v.WMA)
	}
...
func analyzeWMASignal(wmaData []*WMAData, multiWMA map[int][]*WMAData, period string) *Signal {
	if len(wmaData) < 2 {
		return &Signal{Target: TargetWMA, SignalType: "none", Side: SideNone, Period: period, Confidence: 0, Message: "数据不足"}
	}
	...
	return &Signal{Target: TargetWMA, SignalType: "none", Side: SideNone, Period: period, Confidence: 0.5, Message: "未发现明确信号"}
}

func GetWMASignal(klines []Kline, period int, timePeriod string) *Signal {
	// 1.	WMA的参数选择：
	...
	// 分析WMA信号
	signal := analyzeWMASignal(wma10Data, multiWMA, timePeriod)
